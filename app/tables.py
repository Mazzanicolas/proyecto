...
class TestPersonTable(ExportMixin, tables.Table):
    ...
    def render_tiempoViaje(self, record):
        if(self.currentUser == -1 or self.currentUser != record.individuo.id):
            self.tiempos = getTiempos(record.individuo,record.centro,record.individuo.tipo_transporte.id)
...
class SimPersonTable(TestPersonTable):
    ...
    def render_prestadorIndividuo(self, record):
        if(not self.init):
            self.init = True
            setParams(self, CrequestMiddleware.get_request().COOKIES)
            print(CrequestMiddleware.get_request().COOKIES)
        ...
        print('Mutualista: '+str(mutualista)+" Self.Mutualista: "+str(self.mutualista))
    ...
    def render_tiempoViaje(self, record):
        #print(self.init)
        if(self.currentUser == -1 or self.currentUser != record.individuo.id):
            self.tiempos = getTiempos(record.individuo,record.centro,self.tipoTrans)
...
def setParams(self,cookies):
    ...
    print(cookies.get('mutualista','-1'))
def getTiempos(individuo,centro,tipoTrans):
    ...
    print("Tipo de transporte: "+str(tipoTrans))
...
#    print("Llegue")
    print(tiempos)
    print(tieneTrabajo,tieneJardin)
    ...
    if(tiempoViaje > tiempoMaximo or tiempoViaje == -1):
        ...
        return "No"
    if(tieneTrabajo and trabajo and dia in getListOfDays(trabajo.dias)):
        if(hora < trabajo.hora_inicio):
            horaAcumulada += tiempoConsulta
            if(tieneJardin and jardin and dia in getListOfDays(jardin.dias)):
